...
public class CalibrartionName {


    private static final Map<String, String> CONVERSOR = new HashMap(
            "zero", "0",
            "one", "1",
            "two", "2",
            "three", "3",
    ...
    public static void main(String[] args) {
        ...
        for (String linha : linhas) {
            // Converter palavras para números
            String linhaConvertida = converterPalavrasParaNumeros(linha);


            char primeiroDigito = ' ';
            char ultimoDigito = ' ';

            for (int i = 0; i < linhaConvertida.length(); i++) {
                if (Character.isDigit(linhaConvertida.charAt(i))) {
                    primeiroDigito = linhaConvertida.charAt(i);
                    break;
                }
            }

            for (int i = linhaConvertida.length() - 1; i >= 0; i--) {
                if (Character.isDigit(linhaConvertida.charAt(i))) {
                    ultimoDigito = linhaConvertida.charAt(i);
                    break;
                }
            }


            int valorCalibracao = Integer.parseInt("" + primeiroDigito + ultimoDigito);


            somaTotal += valorCalibracao;
        }
    ...
    private static String converterPalavrasParaNumeros(String linha) {
        String linhaConvertida = linha.toLowerCase();
        for (Map.Entry<String, String> entrada : CONVERSOR.entrySet()) {
            linhaConvertida = linhaConvertida.replace(entrada.getKey(), entrada.getValue());
        }
